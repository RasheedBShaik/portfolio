...
import { User, Code2, Layout, Cpu, Globe, Zap, Palette, Layers, Terminal, Sparkles, MoveRight } from "lucide-react";
import { motion } from "framer-motion";

export default function AboutMe() {
  return (
    <section id="about" className="relative w-full py-24 px-6 font-syne bg-[#020204]">
      <div className="mx-auto max-w-6xl relative z-10">
        ...
        >
          ...
          <h2 className="text-5xl font-extrabold tracking-tight text-white md:text-7xl">
            Designing <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500">Interfaces</span><br/>
            Building Systems
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          >
            ...
            <div className="space-y-6 text-lg md:text-xl leading-relaxed text-slate-400">
              <p>
                I am a <span className="text-white font-semibold">Frontend Engineer</span> obsessed with the friction-point between design and engineering. I build high-performance web apps with <span className="text-blue-400 font-medium italic underline underline-offset-4 decoration-blue-500/30">React, Next.js, and TypeScript.</span>
              </p>
              <p>
                My philosophy is simple: <span className="text-white">If it doesn't feel instant, it's not finished.</span> I bridge the gap between complex backend logic and the end-user, ensuring every interaction is meaningful and accessible.
              </p>
            </div>
            
            <div className="mt-12 flex flex-wrap items-center gap-6">
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500">
                    <Globe size={14} className="text-blue-500"/> India / Remote-Available
                </div>
          ...
          >
            ...
            <div className="space-y-7">
              {[
                { label: "Core", val: "Next.js / React 19", color: "from-blue-500 to-cyan-400" },
                { label: "Styling", val: "Tailwind / Motion", color: "from-pink-500 to-rose-400" },
                { label: "Backend", val: "Node / PostgreSQL", color: "from-emerald-500 to-teal-400" },
                { label: "Graphics", val: "Three.js / GSAP", color: "from-purple-500 to-violet-400" },
              ].map((item) => (
                <div key={item.label} className="group/item">
                  <div className="flex justify-between items-end mb-2">
                    <span className="text-[10px] uppercase font-bold tracking-[0.2em] text-slate-500">{item.label}</span>
                    <span className="text-[11px] font-medium text-white/80">{item.val}</span>
                  </div>
...
function FeatureBox({ icon, title, desc, accent }: { icon: React.ReactNode, title: string, desc: string, accent: 'pink' | 'blue' | 'purple' }) {
  const accents = {
  ...
  return (
    ...
    >
      <div className={`mb-4 inline-flex p-3 rounded-2xl bg-white/5 border border-white/10 transition-all ${accents[accent]}`}>
        {icon}
      </div>
      <h4 className="mb-2 font-bold text-white text-xl tracking-tight">{title}</h4>
      <p className="text-sm leading-relaxed text-slate-500 group-hover:text-slate-300 transition-colors">{desc}</p>
    </motion.div>
  );
}